...
import { isDiagnosticReport, extractDiagnosticSummary, formatDiagnosticReport, isConsultationReport, extractConsultationSummary } from "@/lib/diagnosticUtils";
import { DiagnosticReportModal } from "@/components/ui/diagnostic-report-modal";
import { DiagnosisSummary } from "@/components/ui/diagnosis-summary";
import { Bot, User, Send, Mic, Image, FileText, Plus, Music } from "lucide-react";
import drAgroLogo from "@assets/ChatGPT Image 30 de mai. de 2025, 12_06_16_1751801584721.png";
...
export default function Chat() {
  ...
  // Scroll imediato para o final quando seleciona uma conversa
  ...
  // Effect para scroll automático ao selecionar conversa
  useEffect(() => {
    if (selectedConversation && messages?.length > 0) {
      // Reset do estado de scroll manual
  ...
  // Função para limpar caracteres Markdown e formatar texto
  const formatMessageContent = (content: string): string => {
    ...
    return content
      ...
      .replace(/^#{1,6}\s+/gm, '')
      // Remove marcadores de negrito (**texto** ou __texto__)
      .replace(/\*\*(.*?)\*\*/g, '$1')
      .replace(/__(.*?)__/g, '$1')
      // Remove marcadores de itálico (*texto* ou _texto_)
      .replace(/\*(.*?)\*/g, '$1')
      .replace(/_(.*?)_/g, '$1')
      // Remove marcadores de lista (- ou * no início da linha)
      .replace(/^[\*\-]\s+/gm, '• ')
      // Remove marcadores de código (`código`)
      .replace(/`(.*?)`/g, '$1')
      // Remove múltiplas quebras de linha consecutivas
      .replace(/\n{3,}/g, '\n\n')
      // Trim espaços em branco extras
      .trim();
  };
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r flex flex-col">
          ...
          <div className="p-4 border-b bg-green-600 text-white flex-shrink-0">
          ...
          {selectedConversation ? (
            <>
              ...
              <div 
                ...
                className="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-3 bg-gray-50 min-h-0"
                ...
                {messages?.map((message) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Check if this is a diagnosis with modal structure */}
                      {(() => {
                        let parsedContent;
                        try {
                        ...
                        if (message.sender === "ai" && parsedContent?.type === "diagnosis_with_modal") {
                          return (
                            <DiagnosisSummary
                              summary={parsedContent.summary}
                              fullDiagnosis={parsedContent.fullDiagnosis}
                              producer={parsedContent.producer}
                            />
                          );
                        }
                        ...
                        // Check if this is an AI response that should have special rendering
                        if (message.sender === "ai" && conversationMode) {
                          // For diagnosis mode - check if it's a diagnostic report
                          if (conversationMode === "diagnosis" && isDiagnosticReport(message.content)) {
                            return (
                              <div className="space-y-3">
                                ...
                                <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg border border-orange-200 dark:border-orange-800">
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                      <h5 className="font-semibold text-orange-800 dark:text-orange-200 mb-1 text-sm">
                                        Relatório de Diagnóstico
                                      </h5>
                                      <p className="text-orange-700 dark:text-orange-300 text-xs leading-relaxed">
                                        {formatMessageContent(extractDiagnosticSummary(message.content))}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <DiagnosticReportModal 
                                  ...
                                  summary={extractDiagnosticSummary(message.content)}
                                />
                              </div>
                            );
                          }
                        ...
                        return <p className="text-sm whitespace-pre-wrap">{formatMessageContent(message.content)}</p>;
                      })()}
